...
export default function AnalysisPage() {
  ...
  const files = useSelector((state: StoreType) => state.file.list);
  ...
  useEffect(() => {
    ...
    if (files.length === 0) {
      setImgUrl(null);
      setAnalysis(null);
      setError("No handwriting samples found");
      return;
    }

    setImgUrl((files[0] as any).url);
    setLoading(true);
    setError(null);

    dispatch(GetAnalysis(Number(userId)))
  ...
  const handlePrint = useReactToPrint({
    contentRef: printRef,
    documentTitle: 'Analysing Handwriting Report',
  });
